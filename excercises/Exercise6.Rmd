...
BPRSL <- BPRSL %>%
  ...
  mutate(stdbprs = scale(bprs)[,1],
         stdbprs2 = (bprs - mean(bprs))/sd(bprs)) %>%
  ungroup()
...
BPRSL8S1 <- BPRSL8S %>%
  filter(mean > 60) %>% pull(subject)

ggpubr::ggarrange(
  BPRSL %>% ggplot(aes(x = week, y = bprs, color = treatment)) +
    # geom_point(size = 1, position = position_dodge(.5)) +
    stat_summary(geom = "line", position = position_dodge(.5)) +
    stat_summary(position = position_dodge(.5)) +
    theme(legend.position = c(0.8,0.8)) +
  ...
  BPRSL %>%
    filter(!subject %in% BPRSL8S1) %>%
    ggplot(aes(x = week, y = bprs, color = treatment)) +
    # geom_point(size = 1, position = position_dodge(.5)) +
    stat_summary(geom = "line", position = position_dodge(.5)) +
    stat_summary(position = position_dodge(.5)) +
    theme(legend.position = c(0.8,0.8)) +
    scale_y_continuous(name = "mean(bprs) +/- se(bprs)") +
    ggtitle("No. 11 excluded")
)
...
# Factor variables ID and Group
RATS <- RATS %>% mutate(
  ID = factor(ID), Group = factor(Group)
)
# Glimpse the data
glimpse(RATS)
```
