...
Let's read in the `students2014.csv` dataset that was produced in the data wrangling part of the *Assignment 2*.
...
We are dealing with **complete data** with no missing values. Clearly, the four variables `attitude`, `deep`, `stra`, and `surf` represent averaged survey answers on a likert scale. Click here for more comprehensive [metadata](http://www.helsinki.fi/~kvehkala/JYTmooc/JYTOPKYS3-meta.txt). 
...
```{r}
p <- data %>% ggplot(aes(x = age, y = gender, color = gender))
p <- p + ggridges::geom_density_ridges(
  rel_min_height = .001, quantile_lines = T, quantiles = .5, jittered_points = TRUE,
  position = "raincloud", point_alpha = .5, point_size = 1, alpha = .3
)
# p <- p + theme(legend.position = "none")
p <- p + scale_x_sqrt(limits = c(0, NA))
p <- p + labs(title = "Population characteristics")
p <- p + color_gender()
p


# p <- data %>% ggplot(aes(x = gender, y = age))
# p <- p + geom_jitter()
# p
...
Let's begin by checking the distributions of our dataset. 
...
p <- data %>% pivot_longer(
  ...
  values_to = "value",
  names_to = "var"
) %>% ggplot(aes(x = value, color = var, fill = var, y = var, linetype = gender))#
p <- p + ggridges::geom_density_ridges(
  rel_min_height = .001, quantile_lines = T, quantiles = .5, jittered_points = TRUE,
  position = "raincloud", point_alpha = .5, point_size = 1, alpha = .3
)
...
There is plausibility that the `attitude` of the students or different learning strategies influence exam `points`. Therefore, let's glance over these potential relationships:
...
This dataset also contains a variable called `gender`. It could be hypothesized that attitude, learning strategies and/or exam points differ between the genders.
...
p <- data %>% pivot_longer(
  ...
  values_to = "value",
  names_to = "var"
) %>% ggplot(aes(y = value, x = gender, color = gender))#, linetype = gender
p <- p + ggforce::geom_sina(size = 1)
