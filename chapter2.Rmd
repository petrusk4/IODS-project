...
Yup, we got the same result -- the only predictor left in the model is `attitude.` In this model its coefficient is 3.5, which means that each one unit increase in `attitude` corresponds to 3.5 `points` higher exam score. The intercept means that in case `attitude` was 0, the amount of exam `points` would be 11.6. However, this is only theoretical as 0 is outside of the likert scale's range.
...
ggplot(model.diag.metrics, aes(attitude, points)) +
  ...
  geom_segment(aes(xend = attitude, yend = .fitted), color = "red", size = 0.3)
